...
#define DINODE_SIZE 32              //磁盘INode大小
#define SYS_BEGIN 1
#define SYS_SIZE 2
...
struct Super_Block{
    ...
    int ValidData[51];                                          //空闲数据栈 4*51bytes
    int ValidIndex[51];                                         //空闲索引栈 4*51bytes
    ...
    void Super_Block_Init(FILE* disk,bool first=false){
        if(!first && fseek(disk,SUPER_POS,SEEK_SET)==0){
            //栈初始化
            fread(ValidData, sizeof(int),51,disk);
            fread(ValidIndex, sizeof(int),51,disk);
            //起始位置初始化
            ...
            //初始化栈
            memset(ValidData,0,51* sizeof(int));
            memset(ValidIndex,0,51* sizeof(int));
            //初始化起始位置
            user_begin = SUPER_POS+SYS_SIZE;
    ...
    void Super_Block_Back(FILE* disk){
        if(fseek(disk,SUPER_POS,SEEK_SET)==0){
            //栈写回
            fwrite(ValidData, sizeof(int),51,disk);
            fwrite(ValidIndex, sizeof(int),51,disk);
            //起始位置写回
...
private:
    ...
    void FirBloGro(int max,int type,int size);                  //对于第一个结构的特殊分配
    void InitGroup(int& cur,int left,bool type);                //初始化组块
    ...
    void InitDisk(int size=DISK_MAX/BLOCK_SIZE);                //初始化磁盘大小
public:
...
//        std::cout << superBlock;
    ...
    void FreeBlock(int pos);                                    //释放一个块
    void FreeBlocks(const std::vector<int>& blockseq);          //释放多个块
    void BlockCopy(int source,int des);                         //块复制操作
    void BlockExchange(int source,int des);                     //块移动操作
